...
const LOCATION_DATA_URL='https://raw.githubusercontent.com/rukh-debug/location-np/main/english.json';
const LOCATION_ALL_CACHE='bhetinchha_nepal_locations_v053';
let locationDirectory=null, locationLoadPromise=null;

function normalizeLocationDirectory(raw){
  const byDistrict={};
  if(!raw||typeof raw!=='object')return byDistrict;
  Object.values(raw).forEach(province=>{
    if(!province||typeof province!=='object')return;
    Object.entries(province).forEach(([district,municipalities])=>{
      if(!municipalities||typeof municipalities!=='object')return;
      const items=[];
      Object.entries(municipalities).forEach(([name,wards])=>{
        const wardNums=Array.isArray(wards)
          ? wards.map(x=>parseInt(String(x).replace(/\D/g,''),10)).filter(Number.isFinite)
          : [];
        const maxWard=wardNums.length?Math.max(...wardNums):0;
        items.push({name,wardCount:maxWard,wards:wardNums});
      });
      byDistrict[district]=items.sort((a,b)=>a.name.localeCompare(b.name));
    });
  });
  return byDistrict;
}
...
function loadLocationDirectory(){
  ...
  locationLoadPromise=(async()=>{
    try{
      const cached=JSON.parse(localStorage.getItem(LOCATION_ALL_CACHE)||'null');
      if(cached&&cached.data&&Date.now()-cached.t<30*24*60*60*1000){
        locationDirectory=cached.data;
    ...
    try{
      const r=await fetch(LOCATION_DATA_URL,{cache:'force-cache',signal:controller.signal});
      if(!r.ok)throw new Error('location data HTTP '+r.status);
      const raw=await r.json();
      locationDirectory=normalizeLocationDirectory(raw);
      try{localStorage.setItem(LOCATION_ALL_CACHE,JSON.stringify({t:Date.now(),data:locationDirectory}))}catch(e){}
      return locationDirectory;
    }catch(e){
      console.warn('Location directory load failed',e);
      locationDirectory={};
      return locationDirectory;
    }finally{
      clearTimeout(timer);
...
function findDistrictKey(directory,district){
  if(!district||!directory)return '';
  if(directory[district])return district;
  const wanted=String(district).toLowerCase().replace(/[^a-z]/g,'');
  return Object.keys(directory).find(k=>String(k).toLowerCase().replace(/[^a-z]/g,'')===wanted)||'';
}
...
async function municipalitiesFor(district){
  ...
  const key=findDistrictKey(directory,district);
  return key?(directory[key]||[]):[];
}

async function onSearchDistrictChange(){
  const district=$('#city').value, muni=$('#municipalitySelect'), ward=$('#wardSelect');
  fillWardSelect(ward,0);
  if(!district){fillMunicipalitySelect(muni,[],'पहिले जिल्ला छान्नुहोस्');return;}
  muni.disabled=true;
  muni.innerHTML='<option value="">पालिका तयार हुँदैछ...</option>';
  const items=await municipalitiesFor(district);
  fillMunicipalitySelect(muni,items,items.length?'सबै पालिका':'पालिका data उपलब्ध भएन');
}
...
async function onRegDistrictChange(){
  const district=$('#regDistrict').value, muni=$('#regMunicipality'), ward=$('#regWard');
  fillWardSelect(ward,0,'पहिले पालिका छान्नुहोस्');
  if(!district){fillMunicipalitySelect(muni,[],'पहिले जिल्ला छान्नुहोस्');return;}
  muni.disabled=true;
  muni.innerHTML='<option value="">पालिका तयार हुँदैछ...</option>';
  const items=await municipalitiesFor(district);
  fillMunicipalitySelect(muni,items,items.length?'पालिका छान्नुहोस्':'पालिका data उपलब्ध भएन');
}
...
async function doSearch(){
  const btn=$('#searchBtn');
  const original=btn?.innerHTML||'⌕ खोज्नुहोस्';
  const p={
    categoryId:$('#categorySelect').value,
    q:$('#q').value.trim(),
    city:$('#city').value.trim(),
    municipality:$('#municipalitySelect').value.trim(),
    ward:$('#wardSelect').value.trim(),
    radiusKm:+$('#distanceRange').value||5
  };
  if(coords){p.latitude=coords.latitude;p.longitude=coords.longitude;}
  if(btn){btn.disabled=true;btn.innerHTML='⏳ खोज्दै...';}
  $('#resultSummary').textContent='खोजी सुरु भयो...';
  let list=[];
  try{
    ...
    console.warn('Search API',e);
    list=FALLBACK.demoBusinesses||[];
    $('#resultSummary').textContent='Server response ढिलो भयो । कृपया फेरि खोज्नुहोस् ।';
  }finally{
    if(btn){btn.disabled=false;btn.innerHTML=original;}
  }
  if((!C.API_URL||C.DEMO_MODE)&&p.q){
    list=list.filter(x=>(x.name+' '+x.category+' '+x.desc).toLowerCase().includes(p.q.toLowerCase()));
  }
  currentResults=list;
  if(!$('#resultSummary').textContent.includes('ढिलो')){
    $('#resultSummary').textContent=`${list.length} वटा नतिजा${p.q?' • '+p.q:''}${p.city?' • '+p.city:''}${p.municipality?' • '+p.municipality:''}${p.ward?' • वडा '+p.ward:''}`;
  }
  renderResults();
...
function initEvents(){
  ...
  safe('#searchBtn','click',doSearch);
  safe('#q','keydown',e=>{if(e.key==='Enter')doSearch()});
...
async function init(){
  ...
  renderCategorySelect();renderCategories();buildRegisterCategories();
  initEvents();setStep(1);loadLocationDirectory();
  $('#resultSummary').textContent='Category, जिल्ला वा keyword छानेर खोज्नुहोस् ।';
  $('#results').innerHTML='<div class="empty-state"><b>खोज्न तयार छ ।</b><br>माथिको search प्रयोग गर्नुहोस् ।</div>';
  loadBootstrap();
}
document.addEventListener('DOMContentLoaded',()=>{
  try{
    Object.keys(localStorage).filter(k=>k.startsWith('bhetinchha_locations_v052_')).forEach(k=>localStorage.removeItem(k));
  }catch(e){}
  init();
});
